...
public class KeycloakIdentityProvider implements IIdentityProvider {
    ...
    */
    @Inject @RestClient KeycloakRestClient keycloakRestClient;
    ...
    */
    ...
    public void createUser(User user) {
        logger.debug(
                "Request OIDC access token from Keycloak to create user with email: " + user.getEmail());
        KeycloakTokenResponse tokenResponse =
                keycloakRestClient.getToken(realm, "client_credentials", clientId, clientSecret);
        logger.debug("Create user from user data in Keycloak");
        keycloakRestClient.createUser(
                realm, "Bearer " + tokenResponse.access_token, new KeycloakUserRepresentation(user));
    }
}
